...
- **Content blocks**. All deployed content blocks (classes and sub-classes) and their properties/attributes. The content blocks use the objects defined in the domains, augmented with UI (i.e "content") only objects. _E.g. Tax Content Block_.
...
|`ContentBlock/ContentBlock*.rdf`|The list of separate content block ontolgies, one ontology file per content block. Each content block ontology file is included in the root ontology file. The `ContentBlockTemplate.rdf` file should be copied for new content block - see the [guide](https://gov-uk.atlassian.net/wiki/spaces/GIA/pages/edit-v2/5566988289) on how to do this.|
|`ContentType/ContentType*.rdf`|The list of separate content type ontolgies, one ontology file per content type. Each content type ontology file is included in the root ontology file. The `ContentTypeTemplate.rdf` file should be copied for new content block - see the [guide](https://gov-uk.atlassian.net/wiki/spaces/GIA/pages/edit-v2/5566988289) on how to do this.|
